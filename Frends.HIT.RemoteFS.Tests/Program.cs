...
namespace Frends.HIT.RemoteFS.Tests {
  ...
  static class Program {
    ...
    public static async Task Main() {
        ...
        var testSettings = LocalVariables.SpeedadminApiJson;

        var listConfig = new ListParams(){
          Path = "/",
        };

        var serverConfig = new ServerParams{
          ConfigurationSource = ConfigurationType.Json,
          JsonConfiguration = testSettings
        };

        var listFiles = await Frends.HIT.RemoteFS.Main.ListFiles(listConfig, serverConfig);
